...
int frameID = 0;
void vidCb(const sensor_msgs::msg::CompressedImage::ConstSharedPtr img)
{
    cv_bridge::CvImagePtr cv_ptr;
    try
    {
        // Handle both JPEG and H.264 compression
        if (img->format.find("jpeg") != std::string::npos) {
            cv_ptr = cv_bridge::toCvCopy(img, sensor_msgs::image_encodings::BGR8);
        } else if (img->format.find("h264") != std::string::npos) {
            // You may need to add H.264 decoding here
            RCLCPP_WARN(rclcpp::get_logger("vidCb"), "H.264 decoding not implemented");
            return;
        } else {
            RCLCPP_ERROR(rclcpp::get_logger("vidCb"), "Unsupported compression format: %s", img->format.c_str());
            return;
        }
    ...
    {
        RCLCPP_ERROR(rclcpp::get_logger("vidCb"), "cv_bridge exception: %s", e.what());
        return;
    }
    ...
    undistImg->timestamp = img->header.stamp.sec + img->header.stamp.nanosec * 1e-9;
...
{		
	...
	auto imgSub = node->create_subscription<sensor_msgs::msg::CompressedImage>(
		"oakd/rgb/image_raw/compressed", 1, vidCb);
